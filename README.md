...
- Default updater listen address: `127.0.0.1:22268`
...
- Remote updates require an explicit launcher configuration change, for example `GYRE_UPDATE_HOST=0.0.0.0` or a trusted LAN IP in `.gyre_runtime.conf`.
...
Open the launcher menu on the ALAS host and start or restart the updater service. The updater listens on `127.0.0.1` by default; for remote updates, configure `GYRE_UPDATE_HOST` to a trusted LAN address and confirm the port and token match Settings.
